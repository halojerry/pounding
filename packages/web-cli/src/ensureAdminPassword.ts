...
export type EnsureAdminPasswordOptions = {
   ...
   * by launch context — packaged tarball = `pounding-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'pounding-web resetpass';
  ...
  try {
    ...
    deps.warn(`[pounding-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`);
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[pounding-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[pounding-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[pounding-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
    deps.log(`[pounding-web] Generated initial admin password: ${newPassword}`);
    deps.log(`[pounding-web] Log in with username "${username}" and change it from the UI.`);
  } catch (err) {
    deps.warn(
      `[pounding-web] failed to seed initial admin password: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
