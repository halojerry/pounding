...
#   LOG_DIR            Directory for log files (default: ~/Library/Logs/POUNDING)
...
LOG_DIR=${LOG_DIR:-$HOME/Library/Logs/POUNDING}
...
cleanup_labels() {
  ...
  local stale_wts
  stale_wts=$(find /tmp -maxdepth 1 -name 'pounding-pr-*' -type d 2>/dev/null || true)
  if [ -n "$stale_wts" ]; then
