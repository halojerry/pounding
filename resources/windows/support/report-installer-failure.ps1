...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'pounding-installer-fallback-log.jsonl'
}
$logFileName = Split-Path -Leaf $log
$statusPath = Join-Path $env:TEMP 'pounding-installer-report.json'
...
function Read-AionUiAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'POUNDING\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'POUNDING\analytics.json')
  }
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('pounding-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('pounding-handle-' + [guid]::NewGuid().ToString('N') + '.err')
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('POUNDING installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
...
try {
  ...
  Show-ReportMessage ('POUNDING installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Tip: If you can get in touch with the POUNDING team, press [ Ctrl + C ] in this dialog to copy details, then send them to us via social media, email, or a GitHub issue:' + [Environment]::NewLine + 'https://github.com/halojerry/pounding/issues' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('POUNDING installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}
