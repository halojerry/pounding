...
/** Backend manifest id of the built-in POUNDING Butler assistant. */
 ...
 * select the POUNDING Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the POUNDING Butler for you' })
            );
          }
        }
      } catch (error) {
