...
**支持的 Agent：** 内置 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Aion CLI（aionrs，POUNDING 随附的 Rust 后端服务） • Snow CLI • Hermes Agent • Cursor Agent 等
...
不管用哪个模型，Agent 的能力都一样强大——文件读写、网络搜索、图像生成、工具调用，一个不少。POUNDING 支持 **30+ 个 AI 平台**，云端本地都能用。
...
POUNDING 内置 **21 个专业助手**，每个都有独特能力，还能通过自定义技能继续扩展：
...
> [图像生成模型配置指南](https://github.com/iOfficeAI/AionUi/wiki/POUNDING-Image-Generation-Tool-Model-Configuration-Guide)
...
答：<strong>完全不需要。</strong> POUNDING 自带 AI Agent，装好就能用。输入任意 API 密钥即可开始。如果你已经装了 Claude Code 或 Gemini CLI 这些 CLI 工具，POUNDING 会自动发现并集成它们，功能更强大。
</details>
...
答：POUNDING 完全免费且开源。你只需为所选服务商的 API 用量付费，可以使用任何你喜欢的服务商的 API 密钥。
</details>
...
- [🎨 图像生成](https://github.com/iOfficeAI/AionUi/wiki/POUNDING-Image-Generation-Tool-Model-Configuration-Guide) — AI 图像生成
