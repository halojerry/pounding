...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string) => (key === 'cron.status.defaultPrompt' ? 'Create with /cron in POUNDING' : key),
  }),
}));
...
describe('create scheduled task conversation action', () => {
  ...
  it('prefills the current editable conversation without navigating', () => {
    ...
    expect(requestPrefillMock).toHaveBeenCalledWith('current-conversation', 'Create with /cron in POUNDING');
  ...
  it('queues the target-scoped prefill before navigating to a background conversation', () => {
    ...
    expect(requestPrefillMock).toHaveBeenCalledWith('background-conversation', 'Create with /cron in POUNDING');
