...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData POUNDING directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'POUNDING.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'POUNDING.exe' }]);
