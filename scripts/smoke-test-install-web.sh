...
if [[ ! -x "${INSTALL_DIR}/pounding-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/pounding-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/pounding-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/pounding-web"
    exit 1
fi
...
VERSION_OUTPUT=$(pounding-web version 2>&1 || echo "")
