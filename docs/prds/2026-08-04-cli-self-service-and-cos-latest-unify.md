...
- 存量修复：合并后跑一次 `cos-mirror.yml -f tag_name=v2.1.41` 补刷 `latest/`
  （或直接等 v2.1.42 发布时一起刷，视官网是否急需而定）。
...
1. **Linux 下载**：v2.1.41 release 无任何 Linux 资产（无 .deb），官网若生成
   `.deb` 直链必然 404。是否需要补 Linux 构建并纳入发布矩阵，还是官网隐藏
   Linux 下载项？（影响 PR-1 的资产校验范围）
2. **存量 latest/ 补刷时机**：现在就补刷 v2.1.41（官网可用），还是等 v2.1.42
   发布时一次刷好？（当前官网下载页是坏的）
3. `release-distribute.yml` 是否保留（与 `cos-mirror.yml` 功能重叠），还是
   一并收敛为单一入口。
