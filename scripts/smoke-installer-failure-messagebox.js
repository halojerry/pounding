...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    code: 'E1001',
    message: 'POUNDING could not repair the installed uninstaller.',
    action: 'Close POUNDING, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    code: 'E1002',
    message: 'The previous POUNDING uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    code: 'E1003',
    message: 'POUNDING could not remove or replace the previous installation directory.',
    action: 'Close POUNDING and any program using the install folder, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1010',
    message: 'POUNDING could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=POUNDING.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'AIONUI_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'POUNDING cannot continue because the target disk does not have enough free space.',
  ...
  },
  {
    ...
    code: 'E1031',
    message: 'POUNDING installation is incomplete because a required application file is missing.',
    action: 'Reinstall POUNDING or download a newer installer.',
  ...
  },
  {
    ...
    action: 'Download the POUNDING installer that matches this Windows architecture, then run it again.',
  ...
  },
  {
    ...
    code: 'E1050',
    message: 'Another POUNDING installer appears to still be active.',
  ...
  },
  {
    ...
    code: 'E1060',
    message: 'POUNDING found an invalid previous-install registry state.',
    action: 'Uninstall the old POUNDING from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1070',
    message: 'POUNDING could not write the active-installer marker.',
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*POUNDING installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "POUNDING installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "POUNDING Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\POUNDING-messagebox-smoke"
  StrCpy $POUNDINGSessionId "smokembox-${nsisQuote(scenario.code)}"
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`POUNDING installer failure ${code}`)) {
