...
#
# 用法：
#   curl -fsSL https://raw.githubusercontent.com/halojerry/pounding/main/scripts/install-ubuntu.sh | bash
#   # 或指定版本：
...
resolve_version() {
    if [[ -n "${AIONUI_VERSION:-}" ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/halojerry/pounding/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/halojerry/pounding/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
    ...
    DOWNLOAD_URL="https://github.com/halojerry/pounding/releases/download/v${VERSION}/${DEB_FILENAME}"
}
...
Documentation=https://github.com/halojerry/pounding
...
print_summary() {
    ...
    echo -e "  ${BOLD}📖 文件:${NC}  https://github.com/halojerry/pounding"
    echo -e "  ${BOLD}🐛 回報:${NC}  https://github.com/halojerry/pounding/issues"
    echo ""
