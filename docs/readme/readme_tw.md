...
**支援的 Agent：** 內建 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Aion CLI（aionrs，隨附於 POUNDING 的 Rust 後端服務） • Snow CLI • Hermes Agent • Cursor Agent 等
...
不管用哪個模型，Agent 的能力都一樣強大——檔案讀寫、網路搜尋、圖像生成、工具調用，一個不少。POUNDING 支援 **30+ 個 AI 平台**，雲端本地都能用。
...
POUNDING 內建 **21 個專業助手**，每個都有獨特能力，還能透過自訂技能繼續擴展：
...
> [圖像生成模型配置指南](https://github.com/iOfficeAI/AionUi/wiki/POUNDING-Image-Generation-Tool-Model-Configuration-Guide)
...
答：<strong>完全不需要。</strong> POUNDING 自帶 AI Agent，裝好就能用。輸入任意 API 金鑰即可開始。如果您已經裝了 Claude Code 或 Gemini CLI 這些 CLI 工具，POUNDING 會自動發現並整合它們，功能更強大。
</details>
...
答：POUNDING 完全免費且開源。您只需為所選服務商的 API 用量付費，可以使用任何您喜歡的服務商的 API 金鑰。
</details>
...
- [🎨 圖像生成](https://github.com/iOfficeAI/AionUi/wiki/POUNDING-Image-Generation-Tool-Model-Configuration-Guide) — AI 圖像生成
