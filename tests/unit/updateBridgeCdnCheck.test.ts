...
files:
  - url: POUNDING-2.1.45-mac-arm64.zip
    size: 100
  - url: POUNDING-2.1.45-mac-arm64.dmg
    size: 200
path: POUNDING-2.1.45-mac-arm64.zip
...
const GITHUB_RELEASES = [
  {
    ...
    name: 'v2.1.45',
    body: 'changelog body',
    html_url: 'https://github.com/halojerry/pounding/releases/tag/v2.1.45',
    prerelease: false,
    draft: false,
    assets: [],
  },
...
describe('update.check CDN-first', () => {
  ...
  it('reports an update from the CDN manifest and attaches GitHub notes', async () => {
    ...
    expect(res.data?.latest?.htmlUrl).toBe('https://github.com/halojerry/pounding/releases/tag/v2.1.45');
    expect(res.data?.latest?.recommendedAsset?.url).toBe(
      'https://yss-1256275613.cos.ap-guangzhou.myqcloud.com/pounding/releases/latest/POUNDING-2.1.45-mac-arm64.dmg'
    );
  });
