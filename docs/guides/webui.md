...
```cmd
# Using full path
"C:\Program Files\POUNDING\POUNDING.exe" --webui

# Or if POUNDING is in your PATH
POUNDING.exe --webui
```
...
```bash
# Using full path
/Applications/POUNDING.app/Contents/MacOS/POUNDING --webui

# Or using open command
open -a POUNDING --args --webui
```
...
chmod +x POUNDING-*.AppImage

# Run with --webui flag
./POUNDING-*.AppImage --webui
```
...
[Desktop Entry]
Name=POUNDING WebUI
Comment=Start POUNDING in WebUI mode
Exec=/opt/POUNDING/aionui --webui
Icon=aionui
Terminal=false
Type=Application
...
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/POUNDING/aionui --webui --remote
Restart=on-failure
RestartSec=10
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run POUNDING in WebUI mode using Termux with a prooted Linux environment.
...
#### 4. Download and Install POUNDING
...
wget https://github.com/iOfficeAI/AionUi/releases/download/vVERSION/POUNDING_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/AionUi/releases/download/vVERSION/POUNDING_VERSION_arm64.deb

# Install the package
apt install -y ./POUNDING_*.deb
...
```bash
# Start POUNDING in WebUI mode with no-sandbox flag
POUNDING --no-sandbox --webui
```
...
To access POUNDING from other devices on your local network:

```bash
# Start with --remote flag
POUNDING --no-sandbox --webui --remote
...
POUNDING --no-sandbox --webui --port 8080
```
...
chmod +x /opt/POUNDING/aionui
```

#### Out of Memory

POUNDING requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if POUNDING is running: look for "Server started" message
...
4. **Keep device charged** - Running POUNDING consumes battery
...
For convenience, create a script to launch POUNDING quickly:
...
```bash
proot-distro login ubuntu -- bash -c "POUNDING --no-sandbox --webui --remote"
```
...
```cmd
POUNDING.exe --webui --remote
```

### macOS

```bash
/Applications/POUNDING.app/Contents/MacOS/POUNDING --webui --remote
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add POUNDING
...
```powershell
& "C:\Program Files\POUNDING\POUNDING.exe" --webui 2>&1 | Tee-Object -FilePath aionui.log
```
...
| Platform | Location                                                   |
| -------- | ---------------------------------------------------------- |
| Windows  | `%APPDATA%/POUNDING/webui.config.json`                     |
| macOS    | `~/Library/Application Support/POUNDING/webui.config.json` |
| Linux    | `~/.config/POUNDING/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\POUNDING\POUNDING.exe" --resetpass

# Or for a specific user
"C:\Program Files\POUNDING\POUNDING.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/POUNDING.app/Contents/MacOS/POUNDING --resetpass

# Or for a specific user
/Applications/POUNDING.app/Contents/MacOS/POUNDING --resetpass username
```
