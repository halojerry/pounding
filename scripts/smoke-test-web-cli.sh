...
if [ ! -d "$TEMP_DIR/pounding-web" ]; then
  echo "❌ Missing pounding-web directory"
  exit 1
fi

cd "$TEMP_DIR/pounding-web"

# New layout (bun compile standalone binary):
#   pounding-web/
#   ├── pounding-web           ← single compiled executable (no bin/, no dist/, no node_modules)
#   ├── package.json         ← for version lookup
#   ├── static/              ← SPA assets
#   └── bundled-poundingcore/<plat-arch>/...
for dir in static bundled-poundingcore; do
  if [ ! -d "$dir" ]; then
...
if [ ! -x "pounding-web" ]; then
  echo "❌ pounding-web is not executable"
  exit 1
fi
echo "✓ pounding-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./pounding-web version)
if [ -z "$VERSION" ]; then
...
BACKEND_DIR="bundled-poundingcore/$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/aarch64/arm64/; s/x86_64/x64/')"
BACKEND_BINARY="$BACKEND_DIR/poundingcore"
if [ ! -x "$BACKEND_BINARY" ]; then
  echo "❌ Backend binary missing or not executable: $BACKEND_BINARY"
  exit 1
fi
# poundingcore has no --version flag. Read the pinned version from manifest.json
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/pounding-web-data"
...
./pounding-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/pounding-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/pounding-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/pounding-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/pounding-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/pounding-web.html && grep -qE '<(div id="root"|script)' /tmp/pounding-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/pounding-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/pounding-web.html
  echo "---server log---"
  cat /tmp/pounding-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/pounding-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/pounding-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
