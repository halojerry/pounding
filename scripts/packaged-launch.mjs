...
function resolvePackagedApp(projectRoot) {
  ...
  if (process.platform === 'win32') {
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['pounding', 'POUNDING']) {
...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('POUNDING.exe');
    await killProcessByName('POUNDING');
  ...
  const env = {
    ...process.env,
    POUNDING_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] POUNDING_EXTENSIONS_PATH: ${env.POUNDING_EXTENSIONS_PATH}`);
