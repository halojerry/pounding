...
// BEFORE any getPath() call so the whole data tree (config, poundingcore DB, logs)
// lives in a disposable directory. This keeps tests off the developer's real
// database — critical because poundingcore refuses to boot when a shared DB fails
// migration. Guarded by POUNDING_E2E_TEST (set by tests/e2e/fixtures.ts) so it never
// affects dev/production.
...
if (!app.isPackaged) {
  // Skip the dev app name when an E2E sandbox already redirected userData.
  ...
  if (fs.existsSync(portableMarker) && !e2eUserDataDir) {
...
//
// The 9230-9250 reservation and the ~/.pounding-cdp-registry.json instance registry were
...
//
// Override via POUNDING_CDP_PORT env variable. Set to "0" or "false" to disable.
// ---------------------------------------------------------------------------
 ...
 * 顺手删掉遗留的实例注册表文件。
 *
 * 这个文件（~/.pounding-cdp-registry.json）以前记录「每个实例占了哪个 CDP 端口」。相关逻辑
 * 已随应用级 remote-debugging-port 一起删除，但升级上来的机器上文件还在，里面是一堆早已
 * 无效的 pid/端口。留着只会让人以为还有这套机制，所以清掉。best-effort，失败无所谓。
 *
 * Remove the leftover instance-registry file. ~/.pounding-cdp-registry.json used to record
...
function removeLegacyCdpRegistryFile(): void {
  try {
    const legacyRegistry = path.join(os.homedir(), '.pounding-cdp-registry.json');
 ...
 * 用户看不见的 Chrome）。
 *
 * POUNDING_CDP_PORT 现在只当开关用（"0"/"false" 关闭，其它非空值开启）。它的数值不再
 ...
 *
 * POUNDING_CDP_PORT now acts purely as a switch ("0"/"false" disables, any other non-empty
