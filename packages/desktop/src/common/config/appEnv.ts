 ...
 * When POUNDING_MULTI_INSTANCE=1, appends '-2' to isolate the second dev instance.
 * Used to isolate symlink and directory names between environments.
 *
 * @example
 * getEnvAwareName('.pounding')        // release → '.pounding',        dev → '.aionui-dev'
 * getEnvAwareName('.aionui-config') // release → '.aionui-config', dev → '.aionui-config-dev'
 * // with POUNDING_MULTI_INSTANCE=1:  dev → '.aionui-dev-2'
 */
