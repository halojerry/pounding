...
function seedRuntimeKey(
  ...
  // Vendored components (verifyVendorComponents gate): the builtin MCP server.
  writeFile(join(managedResourcesDir, 'mcp', 'chrome-devtools-mcp', 'placeholder'));
...
describe('verifyBundledPoundingcoreResources', () => {
  ...
  it('does not require any CLI in the contract (CLIs are self-served, not bundled)', () => {
    const manifestPath = join(managedResourcesDir, 'manifest.json');
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
  ...
  it('treats a missing CLI executable on disk as warning-only (non-blocking)', () => {
    rmSync(join(managedResourcesDir, 'cli', 'claude'), { recursive: true, force: true });
    ...
    expect(result.failures.filter((f) => f.component === 'claude')).toEqual([]);
    expect(result.missing.filter((m) => m.includes('claude'))).toEqual([]);
  });
  ...
  it('fails when chrome-devtools-mcp vendor dir is missing (builtin MCP offline gate)', () => {
    rmSync(join(managedResourcesDir, 'mcp', 'chrome-devtools-mcp'), { recursive: true, force: true });
    ...
    expect(result.failures).toContainEqual(
      expect.objectContaining({ component: 'managed-resources', reason: 'missing_vendor_mcp' })
    );
  });
