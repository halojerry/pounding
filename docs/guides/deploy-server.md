...
- At least 2GB RAM
- POUNDING `.deb` package from [Releases](https://github.com/iOfficeAI/AionUi/releases)
...
wget https://github.com/iOfficeAI/AionUi/releases/latest/download/POUNDING-linux-amd64.deb

# Install
sudo dpkg -i POUNDING-linux-amd64.deb
...
Create `/opt/POUNDING/start-aionui.sh`:
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "POUNDING is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/POUNDING --webui --remote --no-sandbox \
        > "$LOGFILE" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "POUNDING started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "POUNDING failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "POUNDING is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping POUNDING (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "POUNDING --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "POUNDING stopped."
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "POUNDING is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "POUNDING is not running."
...
```bash
chmod +x /opt/POUNDING/start-aionui.sh
```

> **Tip**: `WORKDIR` determines the directory POUNDING can access for file operations. Set it to your project workspace.
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/POUNDING --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/POUNDING/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
> This proxy is handled by POUNDING's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i POUNDING-linux-amd64.deb`               |
...
- 至少 2GB 内存
- POUNDING `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/AionUi/releases)）
...
创建 `/opt/POUNDING/start-aionui.sh`：
...
start() {
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "POUNDING 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "POUNDING 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "POUNDING 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "POUNDING 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/POUNDING/proxy.pac"`
