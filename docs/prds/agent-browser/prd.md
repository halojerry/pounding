...
**这份 PRD 要回答的问题**：用户让 Agent 上网查东西、办事情时，怎样让整个过程发生在 POUNDING 里——用户看得见、插得上手、登录态不丢。
...
- 该下拉当前在临时工作区与 WebUI 模式下整体隐藏（因为"打开文件夹"在这些场景无意义）。浏览器在临时工作区仍然有意义，因此临时工作区下下拉需保持可见，菜单中仅保留「浏览器」一项，避免出现用户找不到浏览器入口的死角。
  - **WebUI 模式例外**：WebUI 是在普通浏览器里访问 POUNDING，没有 Electron 的 `<webview>` 容器，也没有可供 Agent 操作的 CDP 端口——技术上无法承载应用内浏览器。因此 WebUI 模式下该入口仍然隐藏；这不是遗漏，而是该形态不具备的能力。
...
| 同时开着两个 POUNDING 窗口/实例        | 各自的 Agent 只操作各自实例内的浏览器，互不串扰         |
...
- 用户在应用内浏览器里登录过的网站（Cookie、本地存储），重启 POUNDING 后仍然有效——和普通浏览器一致的预期。
...
- **风险**：本机上的其它程序理论上也能通过该通道操作 POUNDING 内的浏览器页面（仅限本机，不暴露到网络）。业界浏览器自动化工具均采用相同机制，属常规风险水位。
...
1. 全新安装、不做任何配置，对 Agent 说"打开 GitHub 看看 POUNDING 的最新 release"→ 预览框展开、页面呈现、Agent 读到内容并回答。
2. 从文件树面板右上角下拉选「浏览器」→ 输入网址 → 正常浏览；输入关键词 → 搜索结果。
3. Agent 连续打开 3 个不同网站 → 预览框出现 3 个 tab，标题/图标正确，切换流畅。
4. 在某网站登录 → 完全退出 POUNDING 再启动 → 该网站仍是登录态。
